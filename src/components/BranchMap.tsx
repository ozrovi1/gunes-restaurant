...
export function BranchMap({ center, branchName, logoUrl, address }: BranchMapProps) {
  ...
  useEffect(() => {
    ...
    const initMap = async () => {
      ...
      // Fix tile rendering after container is ready
  ...
  return (
    <a
      ...
      className="block relative w-full h-[320px] sm:h-[400px] rounded-2xl overflow-hidden border border-[#d4af37]/20 bg-[#0a1a0a] group cursor-pointer"
    >
      <div
        ref={mapRef}
      ...
      />
      <div
        className="absolute inset-0 pointer-events-none bg-[#0a1a0a]/25 mix-blend-multiply"
        aria-hidden
      />
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 px-5 py-2.5 bg-[#d4af37] text-[#0a0a0a] font-semibold text-xs tracking-[0.2em] uppercase rounded-lg opacity-80 group-hover:opacity-100 transition-opacity pointer-events-none">
        Open in Maps
      </div>
    </a>
  );
