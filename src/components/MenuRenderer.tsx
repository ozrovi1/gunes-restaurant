...
function ItemRow({ item, currency }: { item: MenuItem; currency: string }) {
  ...
  return (
    <div className="py-4 border-b border-[#d4af37]/10 last:border-b-0">
      <div className="flex items-baseline gap-3">
        <h4 className="text-[#faf8f5] font-medium text-[15px] sm:text-base leading-snug">
          {item.name}
          {item.forPeople && (
            <span className="ml-2 text-[10px] sm:text-[11px] text-[#d4af37] tracking-[0.15em] uppercase font-normal">
              For {item.forPeople}
            </span>
          )}
        </h4>
        <span className="flex-1 border-b border-dotted border-[#d4af37]/25 translate-y-[-3px]" aria-hidden />
        {priceStr && (
          <span className="shrink-0 text-[#d4af37] font-semibold text-[15px] sm:text-base tabular-nums">
            {priceStr}
          </span>
        )}
      </div>
        ...
        <div className="mt-2 flex flex-wrap gap-1">
          {item.allergens.map((a) => (
            <span
              ...
              className="inline-flex items-center rounded border border-[#d4af37]/30 bg-[#0d1f0d]/60 px-1.5 py-0.5 text-[9px] font-medium tracking-wider text-[#d4af37]/90 uppercase"
            >
              {a}
            </span>
          ))}
...
export function MenuRenderer({ menu, brandName = "Güneş" }: MenuRendererProps) {
    ...
    <div className="w-full">
          ...
          {categoriesInOrder.map((cat) => {
            ...
            return (
              <button
                ...
                className={
                  isActive
                    ? "inline-flex items-center px-5 py-2.5 rounded-full border-2 border-[#d4af37] bg-[#d4af37] text-[#081408] text-[11px] sm:text-[12px] font-semibold tracking-[0.2em] uppercase shadow-lg shadow-[#d4af37]/20 transition-all duration-200"
                    : "inline-flex items-center px-5 py-2.5 rounded-full border border-[#d4af37]/35 bg-[#0d1f0d]/40 text-[#faf8f5]/80 text-[11px] sm:text-[12px] font-medium tracking-[0.2em] uppercase transition-all duration-200 hover:border-[#d4af37] hover:text-[#d4af37] hover:bg-[#0d1f0d]/70"
                }
              >
                {CATEGORY_LABELS[cat]}
              </button>
      ...
      <OrnamentalFrame className="px-6 py-10 sm:px-12 sm:py-14">
        <div className="text-center mb-10">
          <p className="text-[#d4af37] text-[11px] sm:text-[12px] tracking-[0.3em] uppercase mb-2">{brandName} {branchDisplay}</p>
      ...
      {menu.notices && menu.notices.length > 0 && (
        <div className="mt-12 pt-8 border-t border-[#d4af37]/15 space-y-2 max-w-3xl mx-auto text-center">
      ...
      <div className="mt-6 pt-6 border-t border-[#d4af37]/10 max-w-3xl mx-auto text-center">
        <p className="text-[10px] sm:text-[11px] text-[#d4af37] tracking-[0.25em] uppercase mb-3">Allergen Key</p>
        <div className="flex flex-wrap justify-center gap-x-4 gap-y-1.5 text-[11px] text-[#faf8f5]/55">
          {Object.entries(ALLERGEN_LABELS).map(([code, label]) => (
            <span key={code} className="whitespace-nowrap">
              <strong className="text-[#d4af37]/85 font-semibold">{code}</strong>: {label}
            </span>
          ))}
        </div>
      </div>
